...
annealing, parallel tempering, DFT-guided spectral moves, multi-flip
combinatorial search, Williamson-type symmetric restrictions, and
row-sum-targeted initialization, totalling approximately $5 \times 10^8$
candidate evaluations.  No exact solution was found; all stochastic methods
...
computational searches~\cite{djokovic2018goethals,bright2021sat}---the
...
\begin{enumerate}
  ...
  \item Deploy six distinct optimization strategies within the
    Goethals--Seidel framework, totalling $\sim\!5 \times 10^8$
...
Djokovi\'c and Kotsireas~\cite{djokovic2018goethals,djokovic2009sds,djokovic2008williamson}
...
Kotsireas~\cite{bright2021sat,bright2019applying} developed a SAT+CAS
...
Suksmono~\cite{suksmono2018quantum,suksmono2025qaoa} explored quantum and
quantum-inspired annealing approaches for Hadamard matrix construction,
while Bennett et al.~\cite{bennett2026quaternionic} recently used
...
De~Launey and Gordon~\cite{delauney2014density} showed that the proportion
...
Numba~\cite{bright2019applying} for JIT-compiled inner loops, achieving
$\sim\!31{,}000$ iterations/second for single-flip SA at $n = 167$.  The
...
methods.  The aggregate budget exceeded $5 \times 10^8$ cost evaluations.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    ...
    Standard SA               & $1.5 \times 10^8$ & 403{,}000 &  122 \\
    Parallel tempering        & $1.0 \times 10^8$ & 400{,}000 &  120 \\
    DFT-guided moves          & $8.0 \times 10^7$ & 405{,}000 &  125 \\
    Multi-flip SA             & $7.0 \times 10^7$ & 411{,}000 &  130 \\
    Williamson symmetric      & $5.0 \times 10^7$ & 450{,}000+ & 140+ \\
    Row-sum targeted restarts & $5.0 \times 10^7$ & 401{,}000 &  121 \\
    \midrule
    \textbf{Total}            & $\sim\!5 \times 10^8$ & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  \item Best $\Ltwo$ cost: $\approx 390{,}000$ (target: 0), achieved by
    general SA with multiple random restarts.
  \item Best $\Linf$: $\approx 120$ (target: 0), from parallel tempering.
  \item Legendre baseline: $\Ltwo = 2{,}656$, $\Linf = 4$ (the closest
    near-miss in the~$\Linf$ sense).
  \item Total evaluations: $\sim\!5 \times 10^8$ across all methods.
...
All simulated annealing variants exhibited a three-phase convergence
pattern (Figure~\ref{fig:convergence}):
\begin{enumerate}
  \item \textbf{Rapid descent} (first $\sim\!10^6$ evaluations): cost
    drops from the initial value to~$\approx 4 \times 10^5$.
  \item \textbf{Plateau} ($10^6$--$10^8$ evaluations): cost fluctuates in
    $[4 \times 10^5,\; 4.2 \times 10^5]$.
  \item \textbf{Stagnation} (beyond $10^8$): no further improvement.
\end{enumerate}
The convergence to $\mathcal{C} \approx 4 \times 10^5$ across all methods
and starting points suggests a \emph{structural barrier} in the
optimization landscape rather than a limitation of any particular
algorithm.
...
methods.  General SA and parallel tempering achieved the lowest costs
($\sim\!4 \times 10^5$), while the Williamson-restricted search performed
worst due to the overly constraining palindromic symmetry.
...
Table~\ref{tab:neighbors} compares order~668 with nearby orders.  Both
$H(652) = H(4 \times 163)$ and $H(692) = H(4 \times 173)$ are known, yet
$H(668)$ remains open.  The key differentiator is the subgroup structure
of~$\ZZ_p^*$: for $p = 163$, the order $162 = 2 \times 3^4$ is rich in
subgroups, while for $p = 167$, the order $166 = 2 \times 83$ admits
essentially no useful intermediate subgroups.
...
\begin{enumerate}
  \item No algebraic obstruction is
    known~\cite{eliahou2025modular,delauney2014density}.
  \item The asymptotic density of Hadamard orders approaches~1
    \cite{delauney2014density,delauney2009asymptotic}.
...
\begin{enumerate}
  ...
  \item \textbf{Legendre baseline characterization}: four Legendre
    ...
    of~$E$ are~$-4$.
  \item \textbf{Extensive computational search}: $\sim\!5 \times 10^8$
    evaluations via six distinct optimization strategies, all converging
    to a structural barrier at $\Ltwo \approx 4 \times 10^5$.
...
al.~\cite{bright2021sat,bright2019applying} has proven effective for
...
Quantum annealing~\cite{suksmono2018quantum} and variational quantum
optimization (QAOA)~\cite{suksmono2025qaoa} offer fundamentally different
